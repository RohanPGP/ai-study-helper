        ...
        {cards.map((fc, i) => (
          ...
          >
            {flipped[i] ? (
              ...
              <>
                <p style={{ fontSize: 11, color: 'var(--indigo-600)', fontWeight: 700, marginBottom: 6 }}>QUESTION</p>
        ...
        {questions.map((q, qi) => (
          <div key={qi}>
            ...
            {submitted && q.explanation && (
              <p style={{ fontSize: 13, color: 'var(--indigo-600)', background: 'var(--indigo-50)', padding: '8px 12px', borderRadius: 'var(--radius-sm)', marginTop: 4 }}>
                💡 {q.explanation}
              </p>
            )}
          </div>
...
export default function StudyPackView() {
  ...
  useEffect(() => {
    api.getStudyPack(id)
      .then(({ pack }) => { setPack(pack); setEmailInput(user?.email || ''); })
  ...
  const sendEmail = async () => {
    ...
    setEmailStatus('');
    try {
      await api.sendEmail(id, emailInput);
      ...
      <div className="container" style={{ maxWidth: 800 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <button
              ...
              style={{ background: 'none', border: 'none', color: 'var(--indigo-600)', fontWeight: 600, fontSize: 14, cursor: 'pointer', marginBottom: 8 }}
            >
              ← Dashboard
            </button>
        ...
        <div className="card" style={{ marginBottom: 24, padding: '16px 20px' }}>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
            <span style={{ fontSize: 14, fontWeight: 600, whiteSpace: 'nowrap' }}>📧 Email this pack:</span>
            <input
              className="input"
            ...
            />
            <button className="btn btn-primary btn-sm" onClick={sendEmail} disabled={emailLoading || !emailInput}>
              {emailLoading ? 'Sending…' : 'Send'}
            </button>
          </div>
          {emailStatus === 'success' && (
            <p style={{ fontSize: 13, color: 'var(--emerald-500)', marginTop: 8 }}>✓ Study pack sent successfully!</p>
          )}
          {emailStatus && emailStatus !== 'success' && (
            <p style={{ fontSize: 13, color: 'var(--red-500)', marginTop: 8 }}>Error: {emailStatus}</p>
          )}
        </div>
